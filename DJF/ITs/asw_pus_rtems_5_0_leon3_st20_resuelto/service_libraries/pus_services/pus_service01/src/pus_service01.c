...
tc_accept_report_t pus_service1_tc_acceptation(tc_handler_t *ptc_handler) {
	...
	if (ptc_handler) {
		...
		if (accept_report.cal_packet_err_ctrl
			...
			switch (type) {
						...
						case (2):
							switch (subtype) {
							case (1): // Subtipo 1: On/Off Device
								break;
							default:
						...
						// --- CORRECCIÓN 1: El Servicio 12 debe ser un 'case' independiente ---
						case (12):
							switch (subtype) {
							case (1): // Enable monitoring
							case (2): // Disable monitoring
							case (5): // Check/Define monitoring
								break;
							default:
								accept_report.accept_status = TCAcceptationSubTypeError;
							}
						...
						// --- CORRECCIÓN 2: El Servicio 19 necesita los 'break' internos ---
						case (19):
							switch (subtype) {
							case (1): // Add Event-Action
							case (2): // Delete
							case (4): // Enable
							case (5): // Disable
	...
	error_code_t pus_service1_tx_TM_1_2(tc_handler_t *pvalid_tc_handler,
		...
		if (0 == error) {
			...
			//Request ID ->TM
			error = pus_service1_build_TM_1_X_reqID(pvalid_tc_handler,
					&tm_handler);
