	...
	for (uint16_t i = 0; (i < PUS_SERVICE4_MAX_NUM_OF_STATS) && (!error); i++) {
		...
		if (StatsConfig.defined[i]) {
			...
			if (0 == error) {
				...
				// Min OBT -> TM (Empaquetamos el tiempo en el que ocurrió el mínimo)
				error += tm_handler_append_uint32_appdata_field(&tm_handler,
	...
	if (error) {
		...
		if (1 != N) {
			...
			if (sys_data_pool_is_PID_valid(PID)) {
				...
				if (!error) {
					// 2) Si no hay error (error == 0), generamos el TM[1,7] de éxito
					pus_service1_tx_TM_1_7(ptc_handler);
				} else {
					// 3) Si hay error, generamos el TM[1,8] avisando de que el stat no estaba definido
